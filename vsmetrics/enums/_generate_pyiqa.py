...
def generate_enum_file(default_configs, filename='pyiqa.py'):
    with open(filename, 'w') as f:
        f.write('from enum import Enum\n\n')
        f.write('class MetricEnum(Enum):\n')

        for key, config in default_configs.items():
            metric_type = config['metric_opts']['type'].lower()
            metric_mode = config['metric_mode']
            enum_value = f"('{metric_type}', '{metric_mode}')"

            f.write(f"    {key.upper()} = {enum_value}\n")

    print(f"Enum class written to {filename}")

# https://github.com/chaofengc/IQA-PyTorch/blob/3835ad97e01522d2157e9f5a0cbaf44f66f808fa/pyiqa/default_model_configs.py
...
generate_enum_file(DEFAULT_CONFIGS)
